      ...
      <div className="min-h-full">
        ...
        <div className="relative h-40 overflow-hidden">
          ...
          <div className="relative z-10 px-6 pt-8 flex items-start justify-between">
            <div>
              ...
              <p className="text-white/50 text-sm mt-1">Manage and track all your transactions</p>
            </div>
        ...
        <div className="px-6 pb-8 -mt-2">
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-4">
              <p className="text-xs text-white/40 mb-1">Net Balance</p>
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search transactions..."
                className="w-full pl-9 pr-3 h-9 rounded-lg bg-white/[0.04] border border-white/10 text-white placeholder:text-white/20 text-sm focus:outline-none focus:border-emerald-500/40" />
            </div>
            <div className="flex gap-2">
              {(["all", "income", "expense"] as const).map((t) => (
                <button key={t} onClick={() => setFilterType(t)}
                  className={cn("px-3 py-1.5 rounded-lg text-xs font-medium transition-all",
                    filterType === t ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-white/[0.04] text-white/40 border border-white/[0.06] hover:text-white/60")}>
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl overflow-hidden">
            <div className="hidden md:grid grid-cols-[auto_1fr_auto_auto_auto] gap-4 px-5 py-3 border-b border-white/[0.05] text-xs text-white/30 font-medium uppercase tracking-wider">
            ...
            {isLoading ? [...Array(6)].map((_, i) => (
              ...
              <div key={t.id} className="grid md:grid-cols-[auto_1fr_auto_auto_auto] items-center gap-4 px-5 py-3.5 border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors group">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-white truncate">{t.description || "No description"}</p>
                  {t.notes && <p className="text-xs text-white/30 truncate">{t.notes}</p>}
                </div>
                <span className="text-xs text-white/40 bg-white/[0.04] px-2 py-0.5 rounded-md">{t.categoryName || "—"}</span>
                <span className="text-xs text-white/30">{new Date(t.date).toLocaleDateString()}</span>
                <div className="flex items-center gap-2">
                  <span className={cn("text-sm font-semibold", t.type === "income" ? "text-emerald-400" : "text-white/80")}>
                    {t.type === "income" ? "+" : "-"}{fmt(t.amount)}
                  </span>
                  <button onClick={() => deleteTx.mutate(t.id)} className="text-white/20 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100 ml-1">
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#0d1510] border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-bold text-white text-lg">Add Transaction</h2>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select value={form.categoryId} onChange={(e) => setForm((f) => ({ ...f, categoryId: e.target.value }))}
                      className="w-full h-10 rounded-lg bg-white/[0.04] border border-white/10 text-white text-sm px-3 appearance-none">
