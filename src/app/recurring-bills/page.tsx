...
const STATUS_STYLE: Record<string, { label: string; color: string }> = {
  ...
  upcoming: { label: "Upcoming", color: "text-white/50 bg-white/[0.04] border-white/[0.08]" },
  paused: { label: "Paused", color: "text-white/30 bg-white/[0.02] border-white/[0.06]" },
};
        ...
        <div className="relative h-40 overflow-hidden">
          ...
          <div className="relative z-10 px-6 pt-8 flex items-start justify-between">
            <div>
              ...
              <p className="text-white/50 text-sm mt-1">Track subscriptions and bills so nothing sneaks up on you</p>
            </div>
        ...
        <div className="px-6 pb-8 -mt-2">
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            <div className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-4">
              <p className="text-xs text-white/40 mb-1">Est. Monthly Cost</p>
              <p className="text-xl font-bold text-white">{fmt(monthlyTotal)}</p>
            </div>
            <div className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-4">
              <p className="text-xs text-white/40 mb-1">Tracked Bills</p>
              <p className="text-xl font-bold text-emerald-400">{billsArr.length}</p>
            </div>
            <div className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-4">
              <p className="text-xs text-white/40 mb-1">Due Soon / Overdue</p>
              <p className={cn("text-xl font-bold", dueSoonCount > 0 ? "text-red-400" : "text-white")}>{dueSoonCount}</p>
            </div>
          </div>
          ...
          {!loadingCandidates && (candidates as any[]).length > 0 && (
            <div className="bg-emerald-500/[0.04] border border-emerald-500/20 rounded-xl p-4 mb-6">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="font-semibold text-white text-sm">Detected from your transactions</h2>
              </div>
              <div className="grid md:grid-cols-2 gap-3">
                {(candidates as any[]).map((c) => (
                  <div key={c.key} className="flex items-center justify-between gap-3 bg-white/[0.03] border border-white/[0.06] rounded-lg px-4 py-3">
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-white truncate">{c.name}</p>
                      <p className="text-xs text-white/40 capitalize">{fmt(c.amount)} · {c.frequency} · seen {c.occurrences}x</p>
                    </div>
                    <button
          ...
          {isLoading ? (
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-12 text-center">
              ...
              <p className="text-white/40 text-sm">No recurring bills tracked yet. Add one, or check the detected suggestions above.</p>
            </div>
          ) : (
            <div className="grid md:grid-cols-2 gap-4">
              {billsArr.map((b) => {
                  ...
                  <div key={b.id} className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-5 group hover:border-emerald-500/20 transition-all">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        ...
                        <span className="text-xs text-white/30 capitalize">{b.frequency}{b.categoryName ? ` · ${b.categoryName}` : ""}</span>
                      </div>
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => updateBill.mutate({ id: b.id, data: { isActive: !b.isActive } })} className="text-white/30 hover:text-emerald-400 transition-colors" title={b.isActive ? "Pause" : "Resume"}>
                          {b.isActive ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                        </button>
                        <button onClick={() => deleteBill.mutate(b.id)} className="text-white/30 hover:text-red-400 transition-colors">
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <span className="text-2xl font-bold text-white">{fmt(b.amount)}</span>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {b.isActive && (
                        <button
                          ...
                          className="flex items-center gap-1.5 text-xs bg-white/[0.05] hover:bg-emerald-500/15 text-white/60 hover:text-emerald-400 border border-white/10 hover:border-emerald-500/30 px-3 py-1.5 rounded-lg transition-colors"
                        >
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#0d1510] border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                <h2 className="font-bold text-white text-lg">Add Recurring Bill</h2>
                <button onClick={() => setShowForm(false)} className="text-white/30 hover:text-white"><X className="w-5 h-5" /></button>
              </div>
              <form onSubmit={submit} className="space-y-4">
                <div>
                  <label className="text-white/70 text-sm mb-1.5 block">Bill Name</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <select value={form.frequency} onChange={(e) => setForm((f) => ({ ...f, frequency: e.target.value }))}
                      ...
                      {FREQUENCIES.map((p) => <option key={p} value={p} className="bg-[#0d1510] capitalize">{p}</option>)}
                    </select>
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-white/70 text-sm mb-1.5 block">Next Due Date</label>
                  ...
                  <div>
                    ...
                    <select value={form.categoryId} onChange={(e) => setForm((f) => ({ ...f, categoryId: e.target.value }))}
                      ...
                      {(categories as any[]).filter((c: any) => c.type !== "income").map((c: any) => <option key={c.id} value={c.id} className="bg-[#0d1510]">{c.name}</option>)}
                    </select>
                  </div>
                </div>
