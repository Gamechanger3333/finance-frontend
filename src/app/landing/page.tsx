...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-[#060a08] text-white overflow-x-hidden">
      {/* Navbar */}
      <nav className="fixed top-0 inset-x-0 z-50 border-b border-white/5 bg-[#060a08]/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link href="/login" className="text-sm text-white/70 hover:text-white px-3 py-1.5 rounded-lg transition-colors">Sign in</Link>
        ...
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="/register" className="inline-flex items-center bg-emerald-500 hover:bg-emerald-400 text-white px-8 py-3 rounded-xl font-semibold text-base transition-colors">
                Start for free <ArrowRight className="ml-2 w-4 h-4" />
              </Link>
              <Link href="/login" className="inline-flex items-center border border-white/10 text-white hover:bg-white/5 px-8 py-3 rounded-xl font-semibold text-base transition-colors">
                Try demo account
              </Link>
            </div>
            <p className="mt-4 text-sm text-white/30">No credit card required · Free plan available</p>
          </div>

          <div className="relative">
            <div className="absolute inset-0 bg-emerald-500/10 rounded-2xl blur-3xl scale-95" />
            <div className="relative rounded-2xl overflow-hidden border border-white/10 shadow-2xl">
              ...
              <div className="absolute bottom-4 left-4 right-4 bg-[#0d1510]/90 backdrop-blur-sm border border-white/10 rounded-xl p-4 flex items-center gap-4">
                <div className="w-10 h-10 rounded-lg bg-emerald-500/20 flex items-center justify-center">
                  <TrendingUp className="w-5 h-5 text-emerald-400" />
                </div>
                <div>
                  <p className="text-xs text-white/40">This month&apos;s savings</p>
      ...
      <section id="features" className="py-24 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-white/40 text-lg max-w-2xl mx-auto">From AI-powered insights to automated budgeting, FinFlow has all the tools you need.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {FEATURES.map((f) => (
              <div key={f.title} className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6 hover:border-emerald-500/30 hover:bg-white/[0.05] transition-all duration-200 group">
      ...
      <section id="pricing" className="py-24 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {PLANS.map((plan) => (
              <div key={plan.name} className={`rounded-xl p-6 border relative ${plan.highlight ? "border-emerald-500/40 bg-emerald-500/5" : "border-white/[0.06] bg-white/[0.02]"}`}>
                {plan.highlight && (
                ...
                <Link href="/register" className={`block text-center py-2.5 rounded-lg font-medium text-sm transition-colors ${plan.highlight ? "bg-emerald-500 hover:bg-emerald-400 text-white" : "bg-white/5 hover:bg-white/10 text-white border border-white/10"}`}>
                  {plan.cta}
                </Link>
              </div>
            ))}
      ...
      <section id="testimonials" className="py-24 px-6 border-t border-white/5">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-white/40 text-lg">Here&apos;s what our users say about FinFlow.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {TESTIMONIALS.map((t) => (
              <div key={t.name} className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
      ...
      <section className="py-24 px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-white/40 text-lg mb-8">Join 50,000+ people who&apos;ve transformed their finances with FinFlow.</p>
          <Link href="/register" className="inline-flex items-center bg-emerald-500 hover:bg-emerald-400 text-white px-10 py-3 rounded-xl font-semibold text-base transition-colors">
            Create free account <ChevronRight className="ml-1 w-4 h-4" />
          </Link>
          <p className="mt-4 text-sm text-white/30">Or <Link href="/login" className="text-emerald-400 hover:underline">sign in</Link> to your existing account</p>
        </div>
      </section>
      ...
      <footer className="border-t border-white/5 py-8 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-white/30">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-white/50">FinFlow</span>
          </div>
