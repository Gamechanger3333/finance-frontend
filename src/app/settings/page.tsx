      ...
      <div className="min-h-full">
        ...
        <div>
          <div className="px-6 pt-8 pb-6">
            ...
            <p className="text-foreground/60 text-sm mt-1">Manage your account and preferences</p>
          </div>
        </div>

        <div className="px-6 pb-8">
          <div className="flex gap-1 mb-6 bg-card/70 border border-border rounded-xl p-1 w-fit">
            {TABS.map((t) => (
              <button key={t.id} onClick={() => setTab(t.id as any)}
                className={cn("flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all",
                  tab === t.id ? "bg-accent text-foreground" : "text-muted-foreground hover:text-foreground/60")}>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <div className="bg-card/70 border border-border rounded-xl p-6">
                ...
                <form onSubmit={saveProfile} className="space-y-4">
                  <div>
                    <label className="text-foreground/70 text-sm mb-1.5 block">Full Name</label>
                  ...
                  <div>
                    <label className="text-foreground/70 text-sm mb-1.5 block">Email Address</label>
                    <input value={user?.email} disabled className="w-full h-10 rounded-lg bg-card/50 border border-border/70 text-muted-foreground px-3 text-sm" />
                    <p className="text-xs text-muted-foreground/80 mt-1">Email cannot be changed</p>
                  </div>
                  <div>
                    <label className="text-foreground/70 text-sm mb-1.5 block">Currency</label>
                  ...
                  <div>
                    <label className="text-foreground/70 text-sm mb-1.5 block">Monthly Income Goal ($)</label>
                    <input type="number" value={form.monthlyIncomeGoal} onChange={(e) => setForm((f) => ({ ...f, monthlyIncomeGoal: e.target.value }))}
                      placeholder="e.g. 5000" className="w-full h-10 rounded-lg bg-card border border-border text-foreground placeholder:text-muted-foreground/60 px-3 text-sm focus:outline-none" />
                  </div>
                  <div>
                    <label className="text-foreground/70 text-sm mb-1.5 block">Current Balance ($)</label>
                    <input type="number" step="0.01" value={form.currentBalance} onChange={(e) => setForm((f) => ({ ...f, currentBalance: e.target.value }))}
                      placeholder="e.g. 2500" className="w-full h-10 rounded-lg bg-card border border-border text-foreground placeholder:text-muted-foreground/60 px-3 text-sm focus:outline-none" />
                    <p className="text-xs text-muted-foreground/80 mt-1">Used as the starting point for your Cash-Flow Forecast. Leave blank to estimate it from your transaction history.</p>
                  </div>
              ...
              <div className="space-y-4">
                <div className="bg-card/70 border border-border rounded-xl p-6">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="font-semibold text-foreground">Financial Health Score</h2>
                  </div>
                  <div className="text-center py-4">
                    <div className="relative inline-flex items-center justify-center w-28 h-28">
                      ...
                      <div className="absolute text-center">
                        <p className="text-2xl font-bold text-foreground">{user?.financialHealthScore ?? 0}</p>
                ...
                <div className="bg-card/70 border border-border rounded-xl p-6">
                  <h2 className="font-semibold text-foreground mb-2">Account Type</h2>
                  <p className="text-muted-foreground text-sm capitalize">{user?.userType?.replace("_", " ") ?? "Free"} Plan</p>
                  <div className="mt-4">
          ...
          {tab === "notifications" && (
            <div className="bg-card/70 border border-border rounded-xl overflow-hidden">
              ...
              {(notifications as any[]).length === 0 ? (
                <div className="py-12 text-center text-muted-foreground/80 text-sm">No notifications</div>
              ) : (
                <div className="divide-y divide-border">
                  {(notifications as any[]).map((n: any) => (
                    <button key={n.id} onClick={() => !n.isRead && markRead.mutate(n.id)}
                      className={cn("w-full text-left px-5 py-4 flex items-start gap-3 hover:bg-card/50 transition-colors", !n.isRead && "bg-emerald-500/[0.03]")}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground/60 mt-1">{new Date(n.createdAt).toLocaleDateString()}</p>
                      </div>
          ...
          {tab === "security" && (
            <div className="bg-card/70 border border-border rounded-xl p-6 max-w-lg">
              <h2 className="font-semibold text-foreground mb-5">Security Settings</h2>
              <div className="space-y-4">
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between py-3 border-b border-border last:border-0">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground/80 mt-0.5">{item.status}</p>
                    </div>
                    <button className="px-3 py-1.5 border border-border text-foreground/60 hover:text-foreground hover:bg-card/70 rounded-lg text-xs transition-colors">
                      {item.action}
                    </button>
                  </div>
                ))}
